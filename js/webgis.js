...
const cartoLayer = new ol.layer.Tile({
    title: 'CartoDB Positron', type: 'base', visible: false,
    source: new ol.source.XYZ({ url: 'https://a.basemaps.cartocdn.com/light_all/{z}/{x}/{y}.png' })
});
...
const provinceLayer = new ol.layer.VectorImage({
    title: 'Province Boundaries', visible: true,
...
const GEOSERVER = 'http://localhost:8088/geoserver';
const WORKSPACE = 'gis_lab_group2';

function createWMSLayer(name, title, visible) {
    return new ol.layer.Tile({
        title: title, visible: visible,
        source: new ol.source.TileWMS({
            url: GEOSERVER + '/wms',
            params: { LAYERS: WORKSPACE + ':' + name, TILED: true, FORMAT: 'image/png' },
            serverType: 'geoserver', crossOrigin: 'anonymous'
        })
    });
}

const amacLayer = createWMSLayer('amac_raster', 'PM2.5 Change (AMAC)', true);
const lccLayer = createWMSLayer('lcc_raster', 'LCC (Land Cover Change)', true);
...
const map = new ol.Map({
    ...
    view: new ol.View({
        center: ol.proj.fromLonLat([5.5, 52.2]), zoom: 8, minZoom: 6, maxZoom: 18
    }),
    controls: [
        ...
        new ol.control.MousePosition({
            ...
            }, projection: 'EPSG:4326', className: 'custom-mouse-position'
        }),
        new ol.control.Zoom()
    ]
});
...
function setBaseLayer(name) {
    [osmLayer, cartoLayer, satelliteLayer].forEach(function(l) { l.setVisible(l.get('title') === name); });
}

// --- Layer Toggle ---
const layerMap = {
    'Province Boundaries': provinceLayer,
    'PM2.5 Change (AMAC)': amacLayer,
...
const highlightOverlay = new ol.layer.Vector({
    source: new ol.source.Vector(),
    style: provinceHighlightStyle, zIndex: 10
});
map.addLayer(highlightOverlay);
let highlightFeature = null;
map.on('pointermove', function(evt) {
    const feature = map.forEachFeatureAtPixel(evt.pixel, function(f) { if (f.get('NAME_1')) return f; });
    if (feature !== highlightFeature) {
        highlightFeature = feature;
        highlightOverlay.getSource().clear();
        if (feature) highlightOverlay.getSource().addFeature(feature.clone());
...
const popupOverlay = new ol.Overlay({ element: popupContainer, positioning: 'bottom-center', stopEvent: false, offset: [0, -10] });
map.addOverlay(popupOverlay);
popupCloser.addEventListener('click', function(e) { e.preventDefault(); popupOverlay.setPosition(undefined); return false; });
...
map.on('singleclick', function(evt) {
    ...
    function done() { pending--; if (pending === 0) { popupContent.innerHTML = html; popupOverlay.setPosition(evt.coordinate); } }
    ...
    if (amacUrl) {
        fetch(amacUrl).then(r => r.text()).then(text => {
            ...
            if (m && m.length >= 2) {
                ...
                html += '<tr><td style="padding:2px 6px;font-weight:600;color:#1a5276;">PM2.5 Change</td><td>' + (val !== 'nan' && val !== '' ? parseFloat(val).toFixed(3) : 'No data') + '</td></tr>';
            }
